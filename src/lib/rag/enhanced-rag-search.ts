...
`;
    ...
    prompt += `6. CRITICAL: For EACH violation, you MUST extract the EXACT problematic text directly from the CONTRACT TEXT PROVIDED ABOVE\n`;
    prompt += `7. The "problematicText" field MUST contain actual text that appears in the contract, NOT descriptions or summaries\n`;
    prompt += `8. If a violation is about MISSING text (e.g., no indemnification clause), set problematicText to "MISSING_CLAUSE"\n`;
    prompt += `9. Include the complete sentence or clause containing the issue in "fullClause"\n`;
    prompt += `10. Format response as structured JSON with the following structure:\n`;
    prompt += `   {\n`;
    prompt += `     "violations": [{\n`;
    prompt += `       "type": "violation type",\n`;
    prompt += `       "severity": "CRITICAL|HIGH|MEDIUM|LOW",\n`;
    prompt += `       "description": "detailed description",\n`;
    prompt += `       "problematicText": "EXACT text copied directly from the contract (or MISSING_CLAUSE if not present)",\n`;
    prompt += `       "fullClause": "Complete sentence or paragraph from the contract containing the problematic text",\n`;
    ...
    prompt += `     }]\n`;
    prompt += `   }\n`;
    prompt += `   IMPORTANT: The problematicText MUST be actual text from the contract, not a description!\n`;
